...
const CHARSET = ("日月金木水火土山田口中上下左右大小天地不人心手目耳口车門電氣体数文漢計算機" +
...
export default function MatrixIntro({ onHandoff }) {
    ...
    useEffect(() => {
        ...
        const pickHeadColor = () => {
            const r = Math.random();
            if (r < 0.015)
                return "rgba(255,255,255,0.95)"; // White heads
            if (r < 0.12)
                return MATRIX_GREEN_GLOW; // Bright green
        ...
        const columns = Array.from({ length: cols }, makeCol);
        const t0 = performance.now();
        progressRef.current = 0;
        lastProgressUpdateRef.current = 0;
        const computePct = (now) => {
    ...
    if (phase === "done")
        return null;
    return (_jsxs("div", { className: "fixed bg-black select-none pointer-events-auto", style: {
            position: "fixed",
            top: 0,
            left: 0,
            right: 0,
            ...
            WebkitTransform: "translateZ(0)",
        }, children: [_jsx("canvas", { ref: canvasRef, className: "absolute", style: {
                    position: "absolute",
                    top: 0,
                    left: 0,
                    right: 0,
                    ...
                    transform: "translateZ(0)",
                    WebkitTransform: "translateZ(0)",
                } }), phase === "loading" && (_jsx("div", { className: "absolute inset-0 flex items-center justify-center", children: _jsxs("div", { className: "text-center", children: [_jsxs("div", { className: "text-white text-7xl md:text-8xl font-extrabold tracking-tight", children: [progress, "%"] }), _jsx("div", { className: "mt-3 text-white/90 tracking-[0.4em] text-xs md:text-sm", children: "LOADING" })] }) })), phase === "brand" && (_jsxs("div", { className: "absolute inset-0 flex items-center justify-center", children: [_jsxs("div", { className: "relative overflow-visible text-center px-4", children: [_jsx("img", { src: "/looogo2.png", alt: "DENDRITES", className: "w-[300px] sm:w-[400px] md:w-[500px] lg:w-[600px] xl:w-[700px] h-auto object-contain mx-auto", style: {
                                    animation: "dndx-hyper 1.4s cubic-bezier(.22,1,.36,1) forwards",
                                    filter: "drop-shadow(0 0 20px rgba(255, 255, 255, 0.3))",
                                } }), _jsx("div", { className: "mx-auto mt-2 sm:mt-3 md:mt-4 h-[1.5px] sm:h-[2px] w-[72%] sm:w-[70%] md:w-[72%] dndx-underline" })] }), _jsx("style", { children: `
            .dndx-brand{
              position: relative;
              color: #ffffff;
              animation: dndx-hyper 1.4s cubic-bezier(.22,1,.36,1) forwards;
